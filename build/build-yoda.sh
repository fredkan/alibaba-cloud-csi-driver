...
if [ "$1" == "" ]; then
  version="v1.14"
  cd ${GOPATH}/src/github.com/kubernetes-sigs/alibaba-cloud-csi-driver/build/yoda/
  mv ${GOPATH}/src/github.com/kubernetes-sigs/alibaba-cloud-csi-driver/plugin.csi.alibabacloud.com ./
  docker build -t=registry.cn-hangzhou.aliyuncs.com/plugins/csi-yodaplugin:$version-$GIT_SHA ./
  docker push registry.cn-hangzhou.aliyuncs.com/plugins/csi-yodaplugin:$version-$GIT_SHA
fi
